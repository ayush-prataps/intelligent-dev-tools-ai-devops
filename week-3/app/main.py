...
from app.schemas import (
    AskRequest, AskResponse, RagRequest, RagResponse, CompareRequest,
...
from app.services.rag_service import run_rag_pipeline, run_comparison, retrieve_rag_context
...
def list_chunks(doc_id: Optional[str] = Query(None, description="Filter chunks by document ID")):
    ...
    if not kb: return {"chunks": [], "status": "not_indexed"}
    chunks = kb.get("chunks", [])
    if doc_id: chunks = [chunk for chunk in chunks if chunk.get("doc_id") == doc_id]
    inspected_chunks = []
    for chunk in chunks:
        vector = chunk.get("embedding", [])
        inspected_chunks.append({"chunk_id": chunk.get("chunk_id"), "doc_id": chunk.get("doc_id"), "doc_title": chunk.get("doc_title"), "section": chunk.get("section"), "text": chunk.get("text"), "char_count": chunk.get("char_count"), "word_count": chunk.get("word_count"), "vector_dimension": len(vector), "vector_sample": [round(value, 4) for value in vector[:5]], "full_vector": vector})
...
async def trigger_indexing():
    ...
    return {"status": "success", "message": "Knowledge base indexed successfully.", "summary": result.get("metadata")}
...
def _find_week4_file(filename: str, subfolder: str = "results") -> Optional[str]:
    candidates = [os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "week-4", subfolder, filename)), os.path.abspath(os.path.join(os.path.dirname(__file__), "static", "data", filename)), os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data", "week4", filename)), os.path.abspath(os.path.join("/app", "week-4", subfolder, filename))]
